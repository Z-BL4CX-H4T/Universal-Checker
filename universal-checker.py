...
def process_account(account):
    ...
    if account.startswith("https://") and account.count(':') >= 2:
        try:
            ...
            if result is True:
                print(f"{Fore.GREEN}Login Successful: URL>|{url} • Username>|{username} • Password>|{password}{Style.RESET_ALL}")
                with open("sukses_login.txt", "a") as success_file:
                    success_file.write(f"URL>|{url} • Username>|{username} •  Password>|{password}\n")
            elif result is False:
                print(f"{Fore.LIGHTBLUE_EX}Login Failed: URL>|{url} • Username>|{username} • Password>|{password}{Style.RESET_ALL}")
            else:
                print(f"{Fore.RED}An Error Occurred While Processing Account: URL>|{url} • Username>|{username} • Password>|{password}{Style.RESET_ALL}")
        except ValueError:
